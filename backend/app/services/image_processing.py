...
def analyze_snapshot(req: DetectionRequest) -> DetectionResponse:
    ...
    # Prefer the in-house instance-segmentation model when a trained checkpoint
    # is present; otherwise fall back to the classical OpenCV pipeline. No
    # external services are contacted from either path.
    try:
        from app.services.ml_detector import analyze_snapshot_ml, model_available

        if model_available():
            return analyze_snapshot_ml(
                req=req,
                image=image,
